 ...
 * @origin onz3v
 * @version 1.0.0
 * @description  每天60秒读懂世界 => 兼顾多个平台包括无界
 * @rule ^(60s|每日60)$
 * @admin false
 * @public true
 * @priority 999
 * @disable false
 */
sysMethod.testModule(['got'], { install: true });
sysMethod.testModule(['tough-cookie'], { install: true });
sysMethod.testModule(['iconv-lite'], { install: true });
sysMethod.testModule(['cheerio'], { install: true });
module.exports = async (sender) => {
    const $ = new Env('每天60秒读懂世界')
    const isBncr = typeof sysMethod != 'undefined'
    ...
    async function SendNotify(n, o = '', i = '', t = {}) {
        ...
        if ($.isNode()) {
            if (isBncr) return await sender.reply(`${n}\n${o}\n${c}`)
            try {
    ...
    async function JunLa() {
        const cheerio = $.isNode() ? require('cheerio') : await loadCheerio()
        return new (class {
            constructor() {
